...
from .utils import cmp_fast, cmp_rec, get_indices
import time
...
class GradientReconstructor():
    ...
    def __init__(self, model, mean_std=(0.0, 1.0), config=DEFAULT_CONFIG, num_images=1):
        ...
        if self.config['optim'] == 'LBFGS':
            self.loss_fn = torch.nn.CrossEntropyLoss()
        self.iDLG = True

    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None, cmp=False):
        ...
        scores = torch.zeros(2*self.config['restarts'])
        ...
        try:
            for trial in range(self.config['restarts']):
                ...
                if self.config['mix'] == True:
                    x_mix, labels_mix = self._run_trial(x[trial].clone(), input_data, labels, dryrun=dryrun, cmp=cmp, mix=True)
                    mixlist += x_mix
                    output, labels, process = self._run_trial(mixlist[trial].clone(), input_data, labels, dryrun=dryrun, cmp=cmp)
                else:
                    output, labels, process = self._run_trial(x[trial].clone(), input_data, labels, dryrun=dryrun, cmp=cmp)
                    processes.append(process)
                    outputs.append(output)

                # Finalize
                scores[trial] = self._score_trial(output, input_data, labels)
                #x[trial] = x_trial.detach()
                # if tol is not None and scores[trial] <= tol:
                #     break
                # if dryrun:
                #     break
                if cmp:
                    output, labels, process = self._run_trial(x[trial].clone(), input_data, labels, dryrun=dryrun)
                    outputs.append(output)
                    processes.append(process)
                    # Finalize
                    scores[trial+self.config['restarts']] = self._score_trial(output, input_data, labels)
                    #x[trial+self.config['restarts']] = x_trial
                    # if tol is not None and scores[trial] <= tol:
                    #     break
                    # if dryrun:
                    #     break
        ...
        #outputs = [x_optimal.detach() for x_optimal in x] + mixlist

        return x_optimal.detach(), outputs, stats, processes

    def _init_images(self, img_shape):
        if self.config['init'] == 'randn':
            return torch.randn((2*self.config['restarts'], self.num_images, *img_shape), **self.setup)
        elif self.config['init'] == 'rand':
            return (torch.rand((2*self.config['restarts'], self.num_images, *img_shape), **self.setup) - 0.5) * 2
        elif self.config['init'] == 'zeros':
            return torch.zeros((2*self.config['restarts'], self.num_images, *img_shape), **self.setup)
        else:
            raise ValueError()

    def _run_trial(self, x_trial, input_data, labels, dryrun=False, cmp=False, mix=False):
        ...
        try:
            if cmp:
                indices = get_indices(input_data, cmp)
            for iteration in range(max_iterations):
                closure = self._gradient_closure(optimizer, x_trial, input_data, labels, cmp, mix, indices)
                ...
                with torch.no_grad():
                    ...
                    if (iteration + 1 == max_iterations) or iteration % 500 == 0:
                        ...
                        process.append(x_trial.clone())
                    elif mix and iteration % 20 == 0:
                        print(f'It: {iteration}. Rec. loss: {rec_loss.item():2.4f}.')
                        #outputs.append(x_trial.clone())
                    elif self.config['optim'] == 'LBFGS' and iteration % 50 == 0:
                        print(f'It: {iteration}. Rec. loss: {rec_loss.item():2.4f}.')
                        process.append(x_trial.clone())
                ...
                if dryrun:
                    break
            output = x_trial.clone()
            
        except KeyboardInterrupt:
            print(f'Recovery interrupted manually in iteration {iteration}!')
            pass
        #return x_trial.detach(), labels
        return output, labels, process

    def _gradient_closure(self, optimizer, x_trial, input_gradient, label, cmp, mix, indices = []):
        ...
        def closure():
            ...
            #code by BDXC
            if cmp:
                gradient = cmp_rec(gradient, indices)
            
            if mix:
                rec_loss = reconstruction_costs([gradient], input_gradient,
                                                cost_fn='l2', indices=self.config['indices'],
...
def reconstruction_costs(gradients, input_gradient, cost_fn='l2', indices='def', weights='equal'):
    ...
    if cost_fn == 'l2':
        grad_diff = 0
        for gx, gy in zip(trial_gradient, input_gradient):
